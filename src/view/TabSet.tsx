...
export class TabSet extends React.Component<ITabSetProps, any> {
  ...
  updateVisibleTabs() {
    ...
    if (node.isEnableTabStrip() && this.recalcVisibleTabs) {
      const toolbarWidth = (this.toolbarRef as Element).getBoundingClientRect()
        .width;
  ...
  render() {
    ...
    if (this.showToolbar === true) {
      ...
      toolbar = (
        <div
          ...
          className={cm("flexlayout__tab_toolbar", this.props.node)}
    ...
    if (this.showOverflow === true) {
      tabs.push(
        <button
          ...
          className={cm("flexlayout__tab_button_overflow", this.props.node)}
    ...
    let tabStripClasses = cm("flexlayout__tab_header_outer", this.props.node);
    ...
    if (showHeader) {
      let tabHeaderClasses = cm("flexlayout__tabset_header", this.props.node);
      if (node.isActive()) {
        tabHeaderClasses +=
          " " + cm("flexlayout__tabset-selected", this.props.node);
      }
      ...
      tabStrip = (
        ...
        >
          <div
            ...
            className={cm("flexlayout__tab_header_inner", this.props.node)}
          >
            {tabs}
          </div>
        </div>
    ...
    return (
      <div style={style} className={cm("flexlayout__tabset", this.props.node)}>
        {header}
        {tabStrip}
      </div>
    );
  ...
  onOverflowClick = (
    hiddenTabs: Array<{ name: string; node: TabNode; index: number }>
  ) => {
    ...
    PopupMenu.show(
      element,
      hiddenTabs,
      this.onOverflowItemSelect,
      (defaultClassName: string) =>
        this.props.layout.getClassName(defaultClassName, this.props.node)
    );
  }
  ...
  onMouseDown = (
    ...
    this.props.layout.doAction(
      Actions.setActiveTabset(this.props.node.getId())
    );
